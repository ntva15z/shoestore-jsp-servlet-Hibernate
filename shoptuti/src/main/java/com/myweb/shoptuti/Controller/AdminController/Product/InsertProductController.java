...
@WebServlet(urlPatterns = {"/admin-Insert-Product"})
public class InsertProductController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/views/admin/product/insert.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			dao.insert(entity);
			request.getRequestDispatcher("/admin-List-Product?page=1&maxPageItem=5").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
